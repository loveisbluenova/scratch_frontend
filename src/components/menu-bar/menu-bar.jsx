...
// import aisleschool_logo from './components/menuBar/aisleschool_logo.png'
...
// import { closeAlertWithId, showAlertWithTimeout, showStandardAlert } from '../../reducers/alerts.js';
...
class MenuBar extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get(`http://localhost:8000/api/user/${sessionStorage.getItem("_id")}`, {
            headers: {
    ...
    // logout function
    handleLogout = () => {
        axios.post(`http://localhost:8000/api/auth/logout`, { key: "lgo" }, {
            headers: {
                token: sessionStorage.getItem("token")
            }
        }).then((response) => {
    ...
    handleClickSave = async () => {
        ...
        console.log('tragetsqq ======= === ', this.props.targets)
        let targets = this.props.targets;
        let sprites = [];

        // sprites.push(targets.sprites);

        targets.stage.costumes.map((val) => {
        delete val.asset.data
        })

        targets.stage.sounds.forEach((val) => {
        delete val.asset.data
        })

        Object.entries(targets.sprites).map((elem) => {
            sprites.push(elem[1]);
        });

        sprites.forEach((val) => {
            val.costumes.forEach((item, i) => {
                delete item.asset.data
            });})

        sprites.forEach((val) => {
            val.sounds.forEach((item, i) => {
                delete item.asset.data
            });
        });

        targets.sprites = sprites;
        console.log('tragetsqq ======= === ', targets)

        try {
            const response = await axios.post("http://localhost:8000/api/scratch", {
            ...
            if (response.data.success) {
                // this.props.showStandardAlert();
                // this.props.setProjectChanged();
                this.setState({
                    message: response.data.message,
                    type: "success",
                    show: true
        ...
        console.log('this.props.targets', this.props.targets)
        ...
        return (
            ...
            >
                <div className={styles.mainMenu}>
                        ...
                        {(this.props.canManageFiles) && (
                            ...
                            >
                                ...
                                >
                                    ...
                                    {console.log(this.props, "menu file")}
                                    <MenuSection>
                                        {this.props.canSave && (
                    ...
                    {console.log('this.props ===> 787874', this.props)}
...
const mapDispatchToProps = dispatch => ({
    ...
    // closeAlertsWithId : () => dispatch(closeAlertWithId("saveSuccess")),
    // showStandardAlert : () => dispatch(showStandardAlert("saveSuccess")),
    // setProjectChanged : () => dispatch(setProjectChanged()),
